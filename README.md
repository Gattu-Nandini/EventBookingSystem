...
Running it locally
Import the project into Eclipse as a Dynamic Web Project, target Tomcat 9
Start MySQL (via XAMPP or standalone) and run the schema above in phpMyAdmin
Add the MySQL Connector/J jar to WEB-INF/lib, then add it to the project's Build Path
Update the DB credentials in com.event.util.DBConnection if yours differ from root / no password
Run on Tomcat
Log in as admin / admin123, or sign up as a new student
Known limitations
